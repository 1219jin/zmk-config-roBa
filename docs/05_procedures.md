...
### 複数ファイルの一括新規アップロード（予備手段・Chrome経由）
GitHubの file_upload はホストのファイルパス方式が不可。複数ファイルを一度に上げるときは、
内容をbase64でブラウザに注入してFileを組み立て、input[type=file]にセットしてcommitする。
（コミットの最終クリックは手動になる場合あり）
...
## ナレッジ更新：Claude Code（ローカルgit）運用 ★本命

- ローカルclone：`~/Documents/roBa/zmk-config-roBa`
- origin：`1219jin/zmk-config-roBa`（push先）／ upstream：kumamuk-git（参照のみ・push不可）
- 認証：gh CLI を HTTPS でログイン（SSH鍵は未使用）
- ユーザー設定：user.name=1219jin / user.email=1219.jin@gmail.com（コミット履歴で公開）

手順：「docs/○○ をこう更新して commit & push」と依頼すると、Claude Code がローカルを編集 → commit → push origin main。
許可プロンプトは、表示系コマンドは Yes、push等の送信系は内容を確認してから Yes。
